...
app.get('/api/products', (req, res) => {
  ...
  // localhost:5000/api/products?category=laptops
  // localhost:5000/api/products?<searchQuery>=<value>
  // req.query is an object which hold property pass to search 

  if (req.query.category) {
    const filteredProductsData = ProductsData.filter(
      data => data.category.includes(req.query.category)
    )
    res.send(filteredProductsData)
    return
  ...
  /*
  In the given code snippet, you have an Express.js route defined using app.get. This route handles GET requests to the '/api/products' endpoint. The purpose of the return statement in this code is to terminate the function and prevent further execution of the code after the res.send(filteredProductsData) statement is executed.

  Here's an explanation of how this code works:

  When a GET request is made to '/api/products', the route handler is invoked.

  It first checks if there is a query parameter named 'category' in the request (if (req.query.category)). If the 'category' query parameter exists, it indicates that the client is requesting products filtered by a specific category.

  If the 'category' query parameter is present, the code filters the products from the ProductsData array to include only those that match the specified category. This filtered data is stored in the filteredProductsData variable.

  The res.send(filteredProductsData) statement sends the filtered product data as the response to the client, and then the return statement is used.

  The purpose of return here is to immediately exit the route handler function after sending the response. This ensures that no further code within the route handler is executed, which is important because you don't want the code inside the setTimeout block to execute if a category filter has been applied. If the return statement were not present, the code would continue to execute, and the res.send(ProductsData) inside the setTimeout would send the unfiltered product data to the client after a delay of 3 seconds, which is not the desired behavior when a category filter is applied.
  */
})
